...
function getTerrainType(index: number, radialDistance?: number) {
  ...
  if (radialDistance !== undefined) {
    // Get the hex's ring position for specific terrain assignment
    const hex = Array.from(mainGrid)[index] as CustomHex | VerticalHex
    const ringPosition = hex.ringPosition
